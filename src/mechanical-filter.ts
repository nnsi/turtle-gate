 ...
 * Checks BBO spreads, anomalies, trading halts, and recent volatility.
 *
 * Spread strategy:
 *   - Base: JPX monthly average (BBO_SPREAD_THRESHOLDS in config.ts)
 *   - Override: Real-time bid/ask if available from API
 *   - Adjustment: Stress multiplier from intraday volatility
 *   - Emergency: 3× baseline → skip (§8.7.2)
 */
...
export type FilterResult = {
  ...
  spreadSource: "bid_ask" | "jpx_baseline" | "jpx_stressed";
 ...
 * Priority: (1) live bid/ask → (2) JPX baseline × stress multiplier.
 */
 ...
 * Checks: (1) halt, (2) BBO spread, (3) anomaly, (4) recent volatility.
 */
export function checkTicker(
  ...
  const baseline = BBO_SPREAD_THRESHOLDS[ticker] ?? AVG_SPREAD_BPS;
  const { spreadBps, source, stressMul } = estimateEffectiveSpread(ticker, quote);
  ...
  if (ticker === "1624.T" && spreadBps > MACHINERY_SKIP_BPS) {
    // §8.4.1: 「1624.T機械は当日30 bps超(片道)でスキップ」
    spreadOk = false;
  ...
  const passed = spreadOk && noHalt && noAnomaly && noRecentVolatility;
  ...
  return {
    ...
    checks: { spreadOk, noHalt, noAnomaly, noRecentVolatility },
  };
}
  ...
  return candidates.map((ticker) => {
    ...
    if (!data) {
      ...
      return {
        ...
        checkedAt: new Date().toISOString(),
        reasons: ["データ取得失敗"],
        checks: { spreadOk: false, noHalt: false, noAnomaly: false, noRecentVolatility: false },
      };
    }
    return checkTicker(ticker, data.quote, data.bars);
  });
}
