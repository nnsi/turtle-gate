...
function buildGateHistory(
  rows: { date: string; spread_cost_bps: number | null;
    net_return: number | null; anomaly: number }[],
  avgSpread12m: number,
): GateHistory {
  return {
    entries: rows.map((r) => ({
      ...
      liveVsJpxRatio: (r.spread_cost_bps ?? AVG_SPREAD_BPS) / AVG_SPREAD_BPS,
      netReturn: r.net_return ?? 0,
      systemNormal: r.anomaly !== 1,
    })),
