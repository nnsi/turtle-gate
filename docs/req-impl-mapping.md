...
| 要件 | 状態 | 実装箇所 | 備考 |
|------|------|---------|------|
| 8.4.1 BBOスプレッド判定（09:10時点） | ✕ | — | リアルタイム板情報取得が必要 |
| 8.4.1 流動性条件 | ✕ | — | リアルタイム板情報が必要 |
| 8.4.1 異常値・売買停止検出 | ✕ | — | |
| 8.4.2 銘柄別スプレッド基準値 | ✕ | — | JPX月次データの取得・保持が必要 |
| 8.4.3 見送り理由保存 | ✕ | — | |
...
| 要件 | 状態 | 実装箇所 | 備考 |
|------|------|---------|------|
| 8.7 寄り後価格確認（09:10） | ✕ | — | リアルタイムデータ接続が必要 |
| 8.7.2 BBOスプレッド確認（基準値3倍超でスキップ） | ✕ | — | |
...
> 未実装部分は主にデュアルバンド分岐、リアルタイムデータ・外部API連携（一次機械フィルター、LLM、発注）に集中している。
